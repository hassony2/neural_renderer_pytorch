...
class Rasterize(chainer.Function):
    ...
    def forward_face_index_map_gpu(self):
        ...
        if USE_UNSAFE_IMPLEMENTATION:
            ...
            # for each face
            loop = self.xp.arange(
                self.batch_size * self.num_faces).astype('int32')
            ...
            chainer.cuda.elementwise(
                ...
                'function', )(loop, self.faces, self.face_index_map,
                              self.weight_map, self.depth_map,
                              self.face_inv_map, lock)
            ...
            chainer.cuda.elementwise(
                ...
                ''').substitute(image_size=self.image_size, ),
                'function', )(loop, self.faces, faces_inv)

            # for each pixel
            loop = self.xp.arange(self.batch_size * self.image_size *
            ...
            chainer.cuda.elementwise(
                ...
                ''').substitute(
                    ...
                    return_alpha=int(self.return_alpha),
                    return_depth=int(self.return_depth), ),
                'function', )(loop, self.faces, faces_inv, self.face_index_map,
                              self.weight_map, self.depth_map,
                              self.face_inv_map)
    ...
    def forward_background_gpu(self):
        ...
        if background_color.ndim == 1:
            self.rgb_map = self.rgb_map * mask + (
    ...
    def forward_gpu(self, inputs):
        ...
        # initialize outputs
        self.face_index_map = -1 * self.xp.ones(
            (self.batch_size, self.image_size, self.image_size), 'int32')
        self.weight_map = self.xp.zeros((self.batch_size, self.image_size,
                                         self.image_size, 3), 'float32')
        self.depth_map = self.xp.zeros(self.face_index_map.shape,
                                       'float32') + self.far
        if self.return_rgb:
            self.rgb_map = self.xp.zeros((self.batch_size, self.image_size,
                                          self.image_size, 3), 'float32')
            self.sampling_index_map = self.xp.zeros(
                (self.batch_size, self.image_size, self.image_size,
                 8), 'int32')
            self.sampling_weight_map = self.xp.zeros(
                (self.batch_size, self.image_size, self.image_size,
                 8), 'float32')
        else:
        ...
        if self.return_alpha:
            self.alpha_map = self.xp.zeros((self.batch_size, self.image_size,
                                            self.image_size), 'float32')
        else:
            self.alpha_map = self.xp.zeros(1, 'float32')
        if self.return_depth:
            self.face_inv_map = self.xp.zeros(
    ...
    def backward_depth_map_gpu(self):
        ...
        chainer.cuda.elementwise(
            ...
            ''').substitute(
                num_faces=self.num_faces,
                image_size=self.image_size, ), 'function')(
                    loop,
                    self.faces,
                    self.depth_map,
                    self.face_index_map,
    ...
    def backward_gpu(self, inputs, grad_outputs):
        ...
        self.grad_faces = self.xp.ascontiguousarray(
            self.xp.zeros_like(self.faces, dtype='float32'))
        if self.return_rgb:
            self.grad_textures = self.xp.ascontiguousarray(
                self.xp.zeros_like(self.textures, dtype='float32'))
        else:
        ...
        if self.return_alpha:
            if grad_outputs[1] is not None:
                self.grad_alpha_map = self.xp.ascontiguousarray(
                    grad_outputs[1])
            else:
        ...
        if self.return_depth:
            if grad_outputs[2] is not None:
                self.grad_depth_map = self.xp.ascontiguousarray(
                    grad_outputs[2])
            else:
...
def rasterize(
    ...
    """
    return rasterize_rgbad(faces, textures, image_size, anti_aliasing, near,
                           far, eps, background_color, True, False,
                           False)['rgb']
...
def rasterize_depth(
    ...
    """
    return rasterize_rgbad(faces, None, image_size, anti_aliasing, near, far,
                           eps, None, False, False, True)['depth']
